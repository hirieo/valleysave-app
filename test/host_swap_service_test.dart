...
void main() {
  ...
  group('analyze', () {
    test(
      'cuenta 3 objetos no ignorables (1 Chest + 2 Tree); Weeds excluida',
      () async {
    ...
    test('G1: analyze no escribe ni crea nada', () async {
      ...
      expect(
        entries.length,
        1,
        reason: 'no debe aparecer ninguna carpeta nueva',
      );
    });
    ...
    test(
      'cabaña vacía (sin nombre) no es objetivo válido ⇒ invalidSave',
      () async {
        final result = await service.analyze(
          saveFolderPath: saveFolderPath,
  ...
  group('execute — swap en sitio (spec 007)', () {
    ...
    test(
      ...
      () async {
        final originalId = XmlDocument.parse(
          CoopSaveFixture.mainXml(),
        ).rootElement.findElements('uniqueIDForThisGame').first.innerText;
    ...
    test('G8: no queda _old_tmp ni temporal tras el éxito', () async {
      ...
      expect(
        entries.length,
        1,
        reason: 'ni _old_tmp ni ninguna otra carpeta debe sobrevivir',
      );
    });

    test(
      'G3: el zip de respaldo reproduce el save PRE-swap (roundtrip con importSave)',
      () async {
        final result = await service.execute(
          saveFolderPath: saveFolderPath,
          targetUniqueId: CoopSaveFixture.targetUniqueId,
        ...
        final restoreDir = Directory(
          '${tempDir.path}${Platform.pathSeparator}restore',
        );
        ...
        expect(
          importResult.importedFolderName,
          CoopSaveFixture.originalFolderName,
        );
        ...
        expect(
          ...
          reason:
              'el respaldo es la versión ANTERIOR al swap — Ana sigue siendo anfitriona ahí',
        );
      },
    );

    test(
      'G1: si el zip de respaldo no se puede crear, el original queda intacto',
      () async {
        ...
        expect(
          afterSnapshot,
          beforeSnapshot,
          reason: 'el original nunca se toca si el zip falla',
        );
        final entries = await Directory(savesDir).list().toList();
        expect(entries.length, 1);
      },
    ...
    test('G4: sin archivos *_old en el save resultante', () async {
      ...
      final oldFiles = await Directory(
        saveFolderPath,
      ).list().where((e) => e.path.endsWith('_old')).toList();
    ...
    test('G5b: SaveGameInfo conserva día, estación, año y versión', () async {
      final result = await service.execute(
        saveFolderPath: saveFolderPath,
        targetUniqueId: CoopSaveFixture.targetUniqueId,
        backupsDir: backupsDir,
      ...
      final infoRaw = await File(
        '$saveFolderPath${Platform.pathSeparator}SaveGameInfo',
      ).readAsString();
      final parsed = SaveService.parseSaveGameInfo(
        infoRaw,
        folderName: CoopSaveFixture.originalFolderName,
        lastModified: DateTime(2026, 7, 13),
      );

      expect(parsed, isNotNull);
      final save = parsed!;
      expect(save.playerName, CoopSaveFixture.targetName);
      expect(save.dayOfMonth, 5);
      expect(save.year, 1);
      expect(save.currentSeason, 'spring');
      expect(save.gameVersion, '1.6.15');
    });

    test(
      'G6+G12: roles intercambiados; nombres/IDs/ampliación viajan con la persona',
      () async {
        final result = await service.execute(
          saveFolderPath: saveFolderPath,
          targetUniqueId: CoopSaveFixture.targetUniqueId,
        ...
        expect(result.ok, isTrue);
        final mainRaw = await File(
          mainFilePath(CoopSaveFixture.originalFolderName),
        ).readAsString();
        final players = SaveService.parseFullSave(mainRaw);

        final newHost = players.firstWhere(
          (p) => p.uniqueId == CoopSaveFixture.targetUniqueId,
        );
        final oldHost = players.firstWhere(
          (p) => p.uniqueId == CoopSaveFixture.hostUniqueId,
        );

        expect(newHost.isHost, isTrue, reason: 'Bruno pasa a ser el anfitrión');
        expect(
          newHost.name,
          CoopSaveFixture.targetName,
          reason: 'el nombre no se toca',
        );
    ...
    test(
      'G8b: los objetos reubicados no quedan dentro de la huella nueva',
      () async {
        final result = await service.execute(
          saveFolderPath: saveFolderPath,
          targetUniqueId: CoopSaveFixture.targetUniqueId,
        ...
        final farm = XmlDocument.parse(afterRaw).rootElement
            .findElements('locations')
            .first
            .findElements('GameLocation')
            .firstWhere(
              (l) => l.findElements('name').first.innerText == 'Farm',
            );
          ...
          for (final item in container.findElements('item')) {
            ...
            if (!isWeeds) {
              expect(
                ...
                isFalse,
                reason:
                    'objeto no ignorable en $tx,$ty sigue dentro de la huella nueva',
              );
            }
          }
        }
    ...
    test('G9: boundingBox del cofre reubicado = tile*64', () async {
      ...
      expect(
        movedChest,
        isNotNull,
        reason: 'el cofre reubicado debe seguir existiendo',
      );
      final vec = movedChest!
          .findElements('key')
          .first
          .findElements('Vector2')
          .first;
    ...
    test(
      'G10: itemsToRelocate (analyze) == relocatedCount (execute) sobre el mismo save',
      () async {
        final analysis = await service.analyze(
          saveFolderPath: saveFolderPath,
          targetUniqueId: CoopSaveFixture.targetUniqueId,
    ...
    test(
      'G11: re-parseo con SaveService.parseFullSave — host correcto, nº jugadores intacto',
      () async {
        final result = await service.execute(
          saveFolderPath: saveFolderPath,
          targetUniqueId: CoopSaveFixture.targetUniqueId,
        ...
        expect(
          players.firstWhere((p) => p.isHost).uniqueId,
          CoopSaveFixture.targetUniqueId,
        );
      },
    );
  });
  ...
  group('errores', () {
    test(
      'granja saturada en execute ⇒ nada creado, original intacto, sin zip',
      () async {
        await File(
          mainFilePath(CoopSaveFixture.originalFolderName),
        ).writeAsString(CoopSaveFixture.mainXmlSaturated());
        ...
        expect(
          entries.length,
          1,
          reason: 'ninguna carpeta nueva debe sobrevivir',
        );
        ...
        expect(
          afterSnapshot,
          beforeSnapshot,
          reason: 'el original no se toca ni con fallo',
        );
        ...
        expect(
          ...
          isFalse,
          reason:
              'el zip se crea DESPUÉS de validar el swap — noFreeTile falla antes',
        );
      },
    );

    test(
      'objetivo inexistente en execute ⇒ invalidSave, nada creado',
      () async {
        final result = await service.execute(
          saveFolderPath: saveFolderPath,
          targetUniqueId: 'no-existe',
