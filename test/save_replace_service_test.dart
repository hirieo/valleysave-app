...
void main() {
  ...
  group('replaceSaveFolder — fallos, destino intacto', () {
    ...
    test('archivo principal de 0 bytes → validationFailed, destino intacto', () async {
      ...
      final result = await service.replaceSaveFolder(
        ...
        prepare: (dir) async {
          ...
          await File('${dir.path}$sep$folderName').writeAsString(''); // 0 bytes
        },
      );
    ...
    test(
      'validación POST-swap falla → postValidationFailed, original restaurado',
      () async {
        // El contenido preparado pasa la validación previa, pero un [validate]
        // con estado lo rechaza al re-ejecutarse ya sobre el destino final
        // (simula corrupción que solo se detecta post-swap). El original debe
        // volver desde el rollback, byte a byte.
        await _writeSave(Directory(savesDir), folderName, money: '500');
        final before = await _snapshot(Directory(savesDir));
        ...
        final result = await service.replaceSaveFolder(
          ...
          prepare: (dir) =>
              _writeSave(dir.parent, folderName, money: '999').then((_) => null),
          // true en staging (1ª llamada), false ya en destino (2ª llamada).
          validate: (_) async => (++calls) == 1,
        );

        expect(result.ok, isFalse);
        expect(result.error, ReplaceError.postValidationFailed);
        expect(calls, 2, reason: 'validate se re-ejecuta sobre el destino');
        expect(
          await _snapshot(Directory(savesDir)),
          before,
          reason: 'el original (money=500) vuelve intacto desde el rollback',
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        // recupere. Antes se borraba incondicionalmente → save perdido.
        await _writeSave(Directory(savesDir), folderName, money: '500');

        var renameCalls = 0;
        var validateCalls = 0;
        // 1: destino→rollback (real), 2: staging→destino (real),
        // 3: rollback→destino en la reversión (forzado a fallar).
        final flaky = SaveReplaceService.withRename((src, dest) async {
          renameCalls++;
          if (renameCalls == 3) {
            throw const FileSystemException('reversión forzada a fallar');
          }
          await src.rename(dest);
        });

        final result = await flaky.replaceSaveFolder(
          savesDir: savesDir,
          folderName: folderName,
          backupsDir: backupsDir,
          prepare: (dir) =>
              _writeSave(dir.parent, folderName, money: '999').then((_) => null),
          validate: (_) async => (++validateCalls) == 1, // true staging, false destino
        );
        ...
        // El original (500) sigue vivo en un rollback oculto — NO se borró.
        final rollbacks = await Directory(savesDir)
            .list()
            .where(
              (e) =>
                  e is Directory &&
        ...
        expect(
          rollbacks,
          hasLength(1),
          reason: 'el rollback con el original NUNCA se borra si la reversión falla',
        );

        // Y sweepOrphans lo restaura de verdad (el destino quedó borrado).
        await flaky.sweepOrphans(savesDir, backupsDir: backupsDir);
        final restored = File(
          '$savesDir${Platform.pathSeparator}$folderName${Platform.pathSeparator}SaveGameInfo',
        );
        expect(await restored.exists(), isTrue);
        expect(await restored.readAsString(), contains('<money>500</money>'));
      },
    );
    ...
    test(
      ...
      () async {
        ...
        final otherSavesDir = Directory(
          '${tempDir.path}${Platform.pathSeparator}OtroDisco',
        );
        ...
        final call1 = service.replaceSaveFolder(
          ...
          backupsDir: backupsDir,
          prepare: (dir) async {
            await _writeSave(dir.parent, folderName, money: '1');
            await gate.future;
          },
        );

        // Mientras la primera sigue en curso, la misma partida en OTRA raíz
        // debe poder completarse sin más — nunca `busy`.
        final result2 = await service.replaceSaveFolder(
          savesDir: otherSavesDir.path,
          folderName: folderName,
          backupsDir: backupsDir,
          prepare: (dir) =>
              _writeSave(dir.parent, folderName, money: '2').then((_) => null),
        );
  ...
  group('replaceSaveFolder — fallo del swap final', () {
    test(
      ...
      () async {
        ...
        var renameCalls = 0;
        final flakyService = SaveReplaceService.withRename((src, dest) async {
          renameCalls++;
          if (renameCalls == 2) {
            throw const FileSystemException('fallo forzado de test');
          }
          await src.rename(dest);
        });
        ...
        final result = await flakyService.replaceSaveFolder(
          savesDir: savesDir,
          folderName: folderName,
          backupsDir: backupsDir,
          prepare: (dir) =>
        ...
        expect(
          ...
          3,
          reason:
              'destino→rollback (real) + staging→destino (forzado a fallar) '
              '+ rollback→destino en la reversión (real)',
        );
  ...
  group('sweepOrphans', () {
    ...
    test(
      'destino INVÁLIDO + rollback válido → el rollback sustituye, el roto va a cuarentena',
      () async {
        ...
        await brokenDestination.create(recursive: true);
        await File(
          '${brokenDestination.path}${Platform.pathSeparator}SaveGameInfo',
        ).writeAsString('esto no es xml');
        await File(
          '${brokenDestination.path}${Platform.pathSeparator}$folderName',
        ).writeAsString('main');

        final rollbackRoot = Directory(
          '$savesDir${Platform.pathSeparator}.vs_rollback_orphanBad',
        );
        await _writeSave(rollbackRoot, folderName, money: '321');
        ...
        expect(await rollbackRoot.exists(), isFalse);
        // El destino ahora es el contenido del rollback (321), no el roto.
        final info = File(
          '$savesDir${Platform.pathSeparator}$folderName${Platform.pathSeparator}SaveGameInfo',
        );
        expect(await info.readAsString(), contains('<money>321</money>'));
        // El destino roto no se borró sin más: quedó apartado en cuarentena.
        final quarantine = await Directory(savesDir)
            .list()
            .where(
              (e) => e.path.split(Platform.pathSeparator).last.startsWith(
                '.vs_invalid_',
              ),
            )
            .toList();
    ...
    test(
      'destino inválido + rollback TAMBIÉN inválido → no se toca nada',
      () async {
        final brokenDestination = Directory(
          '$savesDir${Platform.pathSeparator}$folderName',
        );
        ...
        final rollbackRoot = Directory(
          '$savesDir${Platform.pathSeparator}.vs_rollback_orphanBoth',
        );
        await rollbackRoot.create(recursive: true);
        final rollbackSave = Directory(
          '${rollbackRoot.path}${Platform.pathSeparator}$folderName',
        );
        await rollbackSave.create(recursive: true);
        await File(
          '${rollbackSave.path}${Platform.pathSeparator}SaveGameInfo',
        ).writeAsString('también roto');

        await service.sweepOrphans(savesDir, backupsDir: backupsDir);

        // Ninguna de las dos copias se tocó — ni se borró, ni se sustituyó.
        expect(await brokenDestination.exists(), isTrue);
        expect(await rollbackRoot.exists(), isTrue);
        final quarantine = await Directory(savesDir)
            .list()
            .where(
              (e) => e.path.split(Platform.pathSeparator).last.startsWith(
