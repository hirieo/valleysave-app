...
class _FakeAuthClient extends http.BaseClient implements AuthClient {
  ...
  final http.StreamedResponse? Function(http.BaseRequest request, String body)
  _handler;
...
void main() {
  group('uploadBackupZip / listDriveBackups (G6)', () {
    test(
      'G6: sube el zip a ValleySave/Backups/, nunca a la subcarpeta de un save',
      () async {
        ...
        final zip = File(
          '${tempDir.path}${Platform.pathSeparator}Farm_pre-swap_20260101-000000.zip',
        );
        ...
        final client = _FakeAuthClient((request, body) {
          ...
          if (request.method == 'POST' &&
              body.contains('"parents":["backups-id"]')) {
    ...
    test(
      'listDriveBackups filtra por folderName y descarta nombres que no encajan',
      () async {
        final client = _FakeAuthClient((request, body) {
          final q = request.url.queryParameters['q'] ?? '';
          if (request.method == 'GET' && q.contains("name='Backups'")) {
          ...
          if (request.method == 'GET' &&
              q.contains("'backups-id' in parents")) {
            return _json({
              'files': [
                {
                  'id': 'b1',
        ...
        final result = await service.listDriveBackups(
          folderName: 'Farm_443121237',
        );
  ...
  group('listSharedSaveBackups', () {
    test(
      'lista solo backups del folder compartido y marca esa ubicación',
      () async {
        final client = _FakeAuthClient((request, body) {
          final q = request.url.queryParameters['q'] ?? '';
          if (request.method == 'GET' &&
            ...
            return _json({
              'files': [
                {
                  'id': 'shared-backup-id',
                  'name': 'Farm_123_backup_20260713-120000.zip',
                  'size': '4096',
                },
                {
                  'id': 'other-backup-id',
                  'name': 'Other_backup_20260713-120000.zip',
                  'size': '2048',
                },
                {'id': 'readme-id', 'name': 'README.txt', 'size': '20'},
              ],
            });
          }
          return null;
        ...
        expect(result, hasLength(1));
        expect(result.single.sharedDriveFileId, 'shared-backup-id');
        expect(result.single.driveFileId, isNull);
        expect(result.single.sizeBytes, 4096);
      },
    );
  });
}
