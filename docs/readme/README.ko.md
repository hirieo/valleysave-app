...
[![Ko-fi](https://img.shields.io/badge/Ko--fi-%EC%BB%A4%ED%94%BC%20%ED%95%9C%20%EC%9E%94%20%EC%82%AC%EC%A3%BC%EA%B8%B0-29ABE0?logo=ko-fi&logoColor=white)](https://ko-fi.com/hirieo)
...
- 🔒 **프라이버시 우선** — ValleySave는 자신의 `ValleySave/` 폴더와 사용자가 명시적으로 공유한 것만 건드립니다. 공유 농장을 지원하기 위해 Google이 전체 Drive 권한을 요구하지만, 코드는 그 외에는 절대 들여다보지 않습니다.
...
>
> 피에르처럼, 근데 바가지는 안 씌워요 — ValleySave는 무료에, 광고도 구독도 없이 혼자서 7개월간 만들었어요. 세이브 파일 덕 좀 봤다 싶으시면 [Ko-fi에서 커피 한 잔](https://ko-fi.com/hirieo) 사주세요 — 그게 아침에 절 움직이게 하는 힘이거든요. ☕

## 작동 방식

1. **연결** — 내 Google 계정을 연동(OAuth).
