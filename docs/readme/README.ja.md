...
<img src="../screenshots/valleysave-saves-screen.png" alt="ValleySaveのメイン画面：Stardew Valleyのセーブデータのプレイ時間、スキル、このデバイスとGoogle Drive間の同期状態を表示" width="820">
...
- 🔒 **プライバシー第一** — ValleySaveが触れるのは自分の`ValleySave/`フォルダと、あなたが明示的に共有したものだけです。共有農場に対応するためGoogleはDriveのフル権限を要求しますが、コードはそれ以外を一切見ません。
...
> [!NOTE]
> **📱 iOS:** アプリ自体はもう完成しています。足りないのはApple社に公開を許可してもらうための年間99ドルだけ — [Ko-fiで応援してもらえたら](https://ko-fi.com/hirieo)、iOS版も仲間入りできます。
>
> ピエール（八百屋）みたいだけど、値段はふっかけません — ValleySaveは無料、広告なし、サブスクなしで、たった一人で7ヶ月かけて作っています。セーブデータで一度でも助かったことがあれば、[Ko-fiでコーヒーを一杯](https://ko-fi.com/hirieo)おごってもらえると — それが朝起きて続ける原動力になります。☕

## 使い方

1. **接続** — 自分のGoogleアカウントを連携（OAuth）。
