...
class _QueueAuthClient extends http.BaseClient implements AuthClient {
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    if (_i >= _responses.length) {
      throw StateError('No hay más respuestas enlatadas (petición ${_i + 1}: ${request.url})');
    }
...
void main() {
  ...
  group('addSharedSave (G1, G2)', () {
    test('G1: rechaza una carpeta sin SaveGameInfo, sin persistir nada', () async {
    ...
    test('G2: guarda el rol actual (writer) leído de capabilities.canEdit', () async {
      final client = _QueueAuthClient([
        _json({
          'id': 'f1',
          'name': 'Ajena',
      ...
      final prefs = await SharedPreferences.getInstance();
      final stored = jsonDecode(prefs.getString('shared_saves_registry')!) as List;
  ...
  group('listSharedSaves (G3, G4)', () {
    test('G3: refresca el rol contra Drive aunque el registro no cambie', () async {
      SharedPreferences.setMockInitialValues({
        'shared_saves_registry': jsonEncode([
          {'folderId': 'f1', 'folderName': 'Ajena', 'ownerEmail': 'owner@example.com'},
        ]),
      });
      final client = _QueueAuthClient([
        _json({
  ...
  group('removeSharedSave (G8)', () {
    test('G8: no hace ninguna llamada de red, solo borra el registro local', () async {
      SharedPreferences.setMockInitialValues({
        'shared_saves_registry': jsonEncode([
          {'folderId': 'f1', 'folderName': 'Ajena', 'ownerEmail': 'owner@example.com'},
          {'folderId': 'f2', 'folderName': 'Otra', 'ownerEmail': 'otro@example.com'},
        ]),
      });
      final client = _QueueAuthClient([]);
      final service = DriveService(client);
  ...
  group('uploadToSharedSave (G6)', () {
    test('G6: lanza si el rol refrescado no es writer, sin subir nada', () async {
      final client = _QueueAuthClient([
        _json({
          'capabilities': {'canEdit': false},
        }),
  ...
  group('downloadSharedSave (G5)', () {
    test('G5: descarga funciona igual sin importar el rol (delega en downloadSave)',
        () async {
      ...
      final downloaded = File('${tempDir.path}${Platform.pathSeparator}SaveGameInfo');
